...
public class BoardReplyAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		boarddata.setBOARD_PASS(request.getParameter("BOARD_PASS")); // 이름, 비번은 보여져서오고 나머진 숨겨져서(hidden) 옴
		...
		//실수값을 가져올거면 Double.parseDouble / Float.parseFloat쓰면 됨.
		
		
		result = boarddao.boardReply(boarddata); // 한사람의 레코드들이 보내짐
		if (result == 0) {
