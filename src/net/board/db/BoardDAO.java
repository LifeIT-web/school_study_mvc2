...
public class BoardDAO {
	...
	public int getListCount(String cond) {
		...
		try {
			...
			System.out.println("getConnetion()"); // 에러찾는 방법 연결됬는지 확인
	...
	public boolean boardInsert(BoardBean board) {
		...
		try {
			...
			if (rs.next()) {
				num = rs.getInt(1) + 1;
				// getInt(1) 총 글 수인데 , 1은 select의 첫번째 컬럼의 값을 가져와라 (=13 학원 기준)
				// 게시글 올릴때마다 board_num값은 pk라 증가시켜준것
			}else {
				num = 1;
			}
	...
	public int boardReply(BoardBean board) {
		...
		try {
			...
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, re_ref); // re_ref : 관련 글 번호  , re_lev : 답글 레벨
			pstmt.setInt(2, re_seq); // re_seq : 관련 글 출력 순서
			result = pstmt.executeUpdate();
	...
	public boolean isBoardWriter(int num, String pass) {
		...
		try {
			...
			System.out.println("isBoardWriter() 글쓴사람 아님 에러 : " + e);
		} finally {
			try {
				if (rs != null) {
					rs.close();
